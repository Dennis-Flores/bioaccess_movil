...
class MonthSummaryScreen extends StatefulWidget {
  ...
  const MonthSummaryScreen({super.key, required this.user});
...
class _MonthSummaryScreenState extends State<MonthSummaryScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (userId <= 0) {
        setState(() {
          _error = 'ID de usuario inválido para el resumen.';
          _loading = false;
        });
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    final rut = widget.user.rut ?? '';
    final company = widget.user.companyName ?? '';
